...
do
    ...
    do
        echo "type dmwp-no-shuffle np = 5 hop = 2 thr = 1 qc = q_$qc window = 7 slidesize = 5 bucketsize = 100 $i-th loop "
...
do
    for ((i=1; i<=$repeat_time; i++))
    do
        echo "type dmwp-no-shuffle np = $np hop = 2 thr = 1 qc = q_0 window = 7 slidesize = 5 bucketsize = 100 $i-th loop"
